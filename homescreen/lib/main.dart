...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void initState() {
    super.initState();
    // Start a timer for 10 seconds before navigating to HomeScreen
        ...
        child: Column(
          ...
          children: [
            ...
            FeatureCard(
              ...
              onTap: () => _navigateToBlankPage(context, 'Recipe Book'),
            ),
            FeatureCard(
            ...
            FeatureCard(
              imagePath: 'assets/suggestions_icon.png', // Suggestions icon (Add your icon here)
              title: 'SUGGESTIONS',
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: Image.asset(imagePath), // Feature icon
            ),
            SizedBox(width: 16),
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight.bold,
                      fontSize: 18,
                      color: Color(0xFFD2007C), // Title color
                    ),
                  ),
                  SizedBox(height: 4),
                  Text(
...
class _SuggestionsPageState extends State<SuggestionsPage> {
  ...
  Future<void> _fetchNearbyPlaces() async {
    try {
      ...
      String url =
          'https://api.foursquare.com/v3/places/nearby?ll=$latitude,$longitude&radius=5000&limit=20';  // Removed categories
      ...
      if (response.statusCode == 200) {
        ...
        if (data['results'] != null) {
          ...
          for (var place in data['results']) {
            allPlaces.add(place);  // Adding places to the Set (duplicates removed)
          }

          setState(() {
            nearbyPlaces = allPlaces.toList();  // Convert back to list
            isLoading = false;
          });
        }
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Nearby Places to Eat, Shop & Explore', style: TextStyle(fontFamily: 'Teko', fontWeight: FontWeight.bold)),
      ),
      body: isLoading
